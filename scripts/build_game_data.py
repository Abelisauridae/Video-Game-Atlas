...
def load_provider_catalog() -> dict:
    payload = load_json(RAW_DIR / "batocera-provider-catalog.json", default={})
...
def build_database(
    ...
    batocera_library = load_batocera_library()
    if batocera_library:
        systems = [
            enrich_batocera_system(dict(item))
            for item in batocera_library.get("systems", [])
            if isinstance(item, dict)
        ]
        games = [dict(item) for item in batocera_library.get("games", []) if isinstance(item, dict)]
        catalog_context = {
            "kind": "batocera",
            "metadata": batocera_library.get("metadata", {}) if isinstance(batocera_library.get("metadata"), dict) else {},
        }
    else:
    ...
    for system in systems:
        ...
        total_achievements = sum(game["achievements"]["count"] for game in system_games)
        total_points = sum(game["achievements"]["points"] for game in system_games)
        total_leaderboards = sum(game["achievements"]["leaderboards"] for game in system_games)
    ...
    notes = [
        ...
        "Batocera currently documents Arcade Database, theGameDB, ScreenScraper, and IGDB as scraper sources.",
    ]
    sources = [
        {
            "name": "Batocera Scrape From",
            "url": "https://wiki.batocera.org/scrape_from",
            "role": "Documents the scraper sources Batocera exposes in its UI",
        }
    ]

    if catalog_context["kind"] == "batocera":
        notes.extend(
            [
                "The current catalog source is a Batocera gamelist import, using the scraped metadata Batocera already wrote per system.",
                "Only box-art image references are imported by default from Batocera to keep the dataset lightweight.",
                "The normalized output can still preserve original scraper attribution from each Batocera game entry.",
            ]
        )
        sources.extend(
            [
                {
                    "name": "Batocera gamelist.xml import",
                    "url": None,
                    "role": "Primary imported catalog source for systems, games, metadata, and box-art paths",
                },
                {
                    "name": "Batocera systems documentation",
                    "url": "https://wiki.batocera.org/systems",
                    "role": "Reference for the imported Batocera system folders",
                },
            ]
        )
    else:
        if publish_box_art_root or publish_box_art_base_url:
            raise ValueError(
                "Portable box-art publishing currently expects a Batocera import because it stages Batocera image paths."
            )
        notes.extend(
            [
                "RetroAchievements-style exports provide the system hierarchy, while media and metadata attribution can point at Batocera-aligned scraper providers.",
        ...
        sources.extend(
            [
                ...
                },
                {
                    ...
                    "role": "Batocera-supported metadata source requiring Client ID and Client secret",
                },
            ]
        )

    if publish_box_art_root and publish_box_art_base_url:
        ...
        notes.append(
            "This build staged Batocera box art into a publish directory and rewrote the atlas image URLs to a web base URL for portable hosting."
        )
    else:
        asset_strategy = {
            "mode": "source-linked",
    ...
    metadata = {
        ...
        "batoceraProviders": provider_catalog["providers"],
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Build the videogame atlas database and optionally stage Batocera box art for web hosting."
    )
    parser.add_argument(
        "--publish-box-art-root",
        default=None,
        help="Optional local directory where referenced Batocera box art should be copied for upload to a bucket or CDN.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--batocera-roms-root",
        default=None,
        help="Optional Batocera roms root to use when staging box art from a scrubbed Batocera import.",
    )
    ...
    database = build_database(
        ...
        batocera_roms_root=Path(args.batocera_roms_root).expanduser() if args.batocera_roms_root else None,
    )
